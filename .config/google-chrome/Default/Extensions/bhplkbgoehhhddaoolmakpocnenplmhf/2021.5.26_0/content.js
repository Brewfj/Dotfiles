...
function вставитьНашуКнопку() {
	if (г_оРазобранныйАдрес.лМобильнаяВерсия) {
		const узКудаВставлять = document.querySelector('nav[class^="ScTopNavContainer-"] > .tw-align-items-center > .tw-mg-l-05');
		if (!узКудаВставлять) {
			return false;
		}
		...
		<div class="tw5-автоперенаправление tw5-js-удалить tw-align-self-center tw-flex-grow-0 tw-flex-nowrap tw-flex-shrink-0 tw-mg-x-05">
			<div class="tw-inline-flex tw-relative tw5-tooltip__container">
			...
			<style>
				...
				.tw5-tooltip__container:hover .tw5-tooltip
				{
					display: block;
				}
			</style>
