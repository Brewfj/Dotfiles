...
const м_Отладка = (() => {
	...
	function сохранитьТокенТрансляции(сТокенТрансляции) {
		_сТокенТрансляции = ОграничитьДлинуСтроки(сТокенТрансляции, 1e4);
	}
	function СохранитьСписокВариантов(сСписокВариантов) {
		_сСписокВариантов = ОграничитьДлинуСтроки(сСписокВариантов, 5e4);
	}
	function СохранитьСписокСегментов(сСписокСегментов) {
		if (_мСпискиСегментов.length === 10) {
			_мСпискиСегментов.shift();
		}
		_мСпискиСегментов.push(ОграничитьДлинуСтроки(сСписокСегментов, 15e4));
	}
...
const м_Новости = (() => {
	...
	const _мНовости = [ [ '2021.3.7', 'J1010', 'F1068' ], [ '2020.10.30', 'J1066', 'F1067' ], [ '2020.10.5', 'J1010', 'F1065' ], [ '2019.10.9', 'J1010', 'F1064' ], [ '2019.3.17', 'J1010', 'F1063' ], [ '2018.10.28', 'J1010', 'F1062' ], [ '2018.8.17', 'J1010', 'F1060' ], [ '2018.7.30', 'J1010', 'F1059' ], [ '2018.6.27', 'J1010', 'F1058' ], [ '2018.6.12', 'J1010', 'F1057' ], [ '2018.5.18', 'J1010', 'F1049' ], [ '2018.4.24', 'J1036', 'F1048' ], [ '2018.4.6', 'J1010', 'F1047' ], [ '2018.3.17', 'J1010', 'F1046' ], [ '2018.3.4', 'J1041', 'F1042' ], [ '2018.2.17', 'J1010', 'F1044' ], [ '2018.1.7', 'J1010', 'F1043' ], [ '2017.11.6', 'J1010', 'F1037', 'F1038' ], [ '2017.10.22', 'J1010', 'F1023' ], [ '2017.10.14', 'J1010', 'F1020' ], [ '2017.9.11', 'J1010', 'F1018' ], [ '2017.8.8', 'J1035', 'F1017' ], [ '2017.6.23', 'J1010', 'F1014' ], [ '2017.5.29', 'J1010', 'F1013' ], [ '2017.3.31', 'J1031', 'F1012' ], [ '2017.2.26', 'J1030', 'F1011' ], [ ПОЛНАЯ_СПРАВКА, 'J1500', 'F1501', 'F1503', 'F1502', 'F1575', 'F1509', 'F1573', 'F1574', 'F1504', 'F1507' ], [ ПОЛНАЯ_СПРАВКА, 'J1513', 'F1570', 'F1571', 'F1572', 'F1511', 'F1506', 'F1510' ], [ ПОКАЗАТЬ_ОДИН_РАЗ, 'J1054', 'F1501' ], [ ДЛЯ_ПЛАНШЕТА, 'J1055', 'F1056' ], [ ПОКАЗЫВАТЬ_ВСЕГДА, 'J1003', 'F1000' ] ];
...
const м_Список = (() => {
	...
	function РазобратьСписок(лЭтоСписокВариантов, сАбсолютныйАдресСписка, сРазбираемыйСписок) {
		...
		let nTargetDuration, nMediaSequenceNumber, лКонецСписка, кРекламныхСегментов, сТипРекламы, сТокенРекламы, моСегменты, оНовыйСегмент, чTwitchПозицияНачалаСписка, чTwitchПозицияКонцаСписка;
		let лРазрыв, сИмяСегмента, чTwitchПозицияСегмента;
		if (лЭтоСписокВариантов) {
			...
			моСегменты = [];
			оНовыйСегмент = null;
			чTwitchПозицияНачалаСписка = чTwitchПозицияКонцаСписка = чTwitchПозицияСегмента = NaN;
			лРазрыв = false;
		}
		const рвТегИлиАдрес = /^#([^:\r\n]*)(?::(.*))?$|^[^\r\n].*$/gm;
		...
		for (let мсТегИлиАдрес; мсТегИлиАдрес = рвТегИлиАдрес.exec(сРазбираемыйСписок); ) {
			...
			Проверить(сНазваниеТега !== '' && сЗначениеТега !== '');
			switch (сНазваниеТега) {
			  case void 0:
				if (лЭтоСписокВариантов) {
					...
					if (оНовыйСегмент === null) {
						throw `Адрес сегмента без #EXTINF: ${сАдрес}`;
					}
					...
					оНовыйСегмент.чTwitchПозиция = чTwitchПозицияСегмента;
					моСегменты.push(оНовыйСегмент);
					лРазрыв = false;
					чTwitchПозицияСегмента += оНовыйСегмент.чДлительность;
			  ...
			  case 'EXTINF':
				...
				сИмяСегмента = чЗапятая === -1 ? '' : сЗначениеТега.slice(чЗапятая + 1);
				break;
			  ...
			  case 'EXT-X-TWITCH-ELAPSED-SECS':
				Проверить(!лЭтоСписокВариантов);
				Проверить(Number.isNaN(чTwitchПозицияНачалаСписка));
				Проверить(моСегменты.length === 0);
				чTwitchПозицияНачалаСписка = чTwitchПозицияСегмента = РазобратьПоложительноеЧисло(сЗначениеТега);
				break;

			  case 'EXT-X-TWITCH-TOTAL-SECS':
				Проверить(!лЭтоСписокВариантов);
				Проверить(Number.isNaN(чTwitchПозицияКонцаСписка));
				Проверить(моСегменты.length === 0);
				чTwitchПозицияКонцаСписка = РазобратьПоложительноеЧисло(сЗначениеТега);
				break;

			  case 'EXT-X-DATERANGE':
				Проверить(!лЭтоСписокВариантов);
				const амАтрибуты = РазобратьСписокАтрибутов(сЗначениеТега);
				if (амАтрибуты.has('X-TV-TWITCH-AD-ROLL-TYPE') && амАтрибуты.has('X-TV-TWITCH-AD-RADS-TOKEN')) {
					сТипРекламы = РазобратьСтроку(амАтрибуты.get('X-TV-TWITCH-AD-ROLL-TYPE'));
					сТокенРекламы = РазобратьСтроку(амАтрибуты.get('X-TV-TWITCH-AD-RADS-TOKEN'));
					м_Журнал.Окак(`[Список] Найден рекламный ролик ${сТипРекламы} ${сТокенРекламы.slice(-10)} ${амАтрибуты.get('X-TV-TWITCH-AD-POD-POSITION')}/${амАтрибуты.get('X-TV-TWITCH-AD-POD-LENGTH')} ${амАтрибуты.get('DURATION')} секунд`);
				}
				break;
		...
		if (лЭтоСписокВариантов) {
			...
			м_Журнал.Вот(`[Список] Количество вариантов в списке: ${моСписокВариантов.length}`);
			return моСписокВариантов;
		} else {
			Проверить(оНовыйСегмент === null);
			Проверить(nTargetDuration !== -1);
			м_Журнал[кРекламныхСегментов === 0 ? 'Вот' : 'Окак'](`[Список] Разобран список сегментов TargetDuration=${nTargetDuration} MediaSequenceNumber=${nMediaSequenceNumber} ПозицияНачала=${чTwitchПозицияНачалаСписка} ПозицияКонца=${чTwitchПозицияКонцаСписка} РекламныхСегментов=${кРекламныхСегментов} КоличествоСегментов=${моСегменты.length}`);
			if (Math.abs(чTwitchПозицияСегмента - чTwitchПозицияКонцаСписка) > .001) {
				м_Журнал.Ой(`[Список] Отклонение позиции конца списка ${чTwitchПозицияСегмента - чTwitchПозицияКонцаСписка}с`);
			}
	...
	function РазобратьСтроку(сИсходныйТекст) {
		Проверить(сИсходныйТекст.length > 2 && сИсходныйТекст[0] === '"' && сИсходныйТекст[сИсходныйТекст.length - 1] === '"');
	...
	function добавитьСегментыВОчередьИПолучитьПараметрыОбновленияСписков(оНовыйСписок) {
		const ИНТЕРВАЛ_ОБНОВЛЕНИЯ_СПИСКА_С_РЕКЛАМОЙ = 2e3;
		const лПоследнийСписокЗаканчиваетсяРекламой = этотСписокЗаканчиваетсяРекламой(_оПоследнийСписок);
		const лНовыйСписокЗаканчиваетсяРекламой = этотСписокЗаканчиваетсяРекламой(оНовыйСписок);
		м_Twitch.отправитьДанныеСлеженияЗаРекламой(оНовыйСписок, лНовыйСписокЗаканчиваетсяРекламой && !оНовыйСписок.лКонецСписка);
		let кСегментовДобавлено = 0;
		if (лПоследнийСписокЗаканчиваетсяРекламой && лНовыйСписокЗаканчиваетсяРекламой || оНовыйСписок.моСегменты.length === 0) {
			м_Статистика.ДобавленыСегментыВОчередь(0, 0);
		} else {
			кСегментовДобавлено = ДобавитьСегментыВОчередь(оНовыйСписок, лПоследнийСписокЗаканчиваетсяРекламой);
		}
		if (оНовыйСписок.лКонецСписка) {
			throw 'Найден #EXT-X-ENDLIST';
		}
		if (лНовыйСписокЗаканчиваетсяРекламой) {
			if (!лПоследнийСписокЗаканчиваетсяРекламой) {
				_оПоследнийСписок = оНовыйСписок;
				м_События.ПослатьСобытие('список-началорекламы');
			} else {
				м_Журнал.Окак('[Список] Продолжение рекламы');
			}
			return ИНТЕРВАЛ_ОБНОВЛЕНИЯ_СПИСКА_С_РЕКЛАМОЙ;
		} else {
			if (лПоследнийСписокЗаканчиваетсяРекламой) {
				_оПоследнийСписок = оНовыйСписок;
				м_События.ПослатьСобытие('список-конецрекламы');
			}
			return ПолучитьИнтервалОбновленияСпискаСегментов(кСегментовДобавлено);
		}
	}
	function ДобавитьСегментыВОчередь(оНовыйСписок, лКонецРекламы) {
		...
		if (лКонецРекламы) {
			...
			let чПозицияПередРекламой = -1;
			for (let ы = _оПоследнийСписок.моСегменты.length; --ы >= 0; ) {
				if (!_оПоследнийСписок.моСегменты[ы].лРеклама) {
					чПозицияПередРекламой = _оПоследнийСписок.моСегменты[ы].чTwitchПозиция;
					break;
				}
			}
			м_Журнал.Окак(`[Список] Позиция перед рекламой: ${чПозицияПередРекламой}`);
			let чРазмерБуфера = м_Настройки.Получить('чРазмерБуфера');
			for (чИндекс = оНовыйСписок.моСегменты.length; --чИндекс >= 0; ) {
				if (оНовыйСписок.моСегменты[чИндекс].чTwitchПозиция <= чПозицияПередРекламой) {
					м_Журнал.Окак(`[Список] Отбор сегментов остановлен на позиции ${оНовыйСписок.моСегменты[чИндекс].чTwitchПозиция}`);
					чИндекс++;
					break;
				}
				if (оНовыйСписок.моСегменты[чИндекс].лРеклама) {
					м_Журнал.Окак(`[Список] Отбор сегментов остановлен из-за рекламы`);
					break;
				}
				чРазмерБуфера -= оНовыйСписок.моСегменты[чИндекс].чДлительность;
				if (чРазмерБуфера <= 0) {
					м_Журнал.Окак(`[Список] Отбор сегментов остановлен из-за размера буфера`);
					break;
				}
			}
			чИндекс = Math.max(чИндекс, 0);
		} else if (_оПоследнийСписок === null) {
			if (оНовыйСписок.лКонецСписка) {
				return 0;
			}
			ДобавитьСегментыВОчередь.лРазрыв = true;
		...
		for (let оДобавляемыйСегмент; оДобавляемыйСегмент = оНовыйСписок.моСегменты[чИндекс]; ++чИндекс) {
			...
			if (чНомерДобавляемогоСегмента >= чНачалоПоследнегоСписка && чНомерДобавляемогоСегмента < чКонецПоследнегоСписка) {
				const {сАдрес, чДлительность} = _оПоследнийСписок.моСегменты[чНомерДобавляемогоСегмента - чНачалоПоследнегоСписка];
				if (сАдрес !== оДобавляемыйСегмент.сАдрес || чДлительность !== оДобавляемыйСегмент.чДлительность) {
					м_Журнал.Ой(`[Список] У сегмента ${чНомерДобавляемогоСегмента} изменились адрес ${сАдрес} ==> ${оДобавляемыйСегмент.сАдрес} или длительность ${чДлительность} ==> ${оДобавляемыйСегмент.чДлительность}`);
					ДобавитьСегментыВОчередь.лРазрыв = true;
				}
			}
			if (чНомерДобавляемогоСегмента >= чКонецПоследнегоСписка) {
				...
				if (оДобавляемыйСегмент.лРеклама) {
					м_Журнал.Окак(`[Список] Не добавляю рекламу MediaSequenceNumber=${чНомерДобавляемогоСегмента}`);
					ДобавитьСегментыВОчередь.лРазрыв = true;
				} else if (оДобавляемыйСегмент.чДлительность === 0) {
					м_Журнал.Ой(`[Список] Не добавляю сегмент MediaSequenceNumber=${чНомерДобавляемогоСегмента} Длительность=0`);
					ДобавитьСегментыВОчередь.лРазрыв = true;
					...
					м_Журнал.Вот(`[Список] Добавлен сегмент ${оДобавлено.чНомер} MediaSequenceNumber=${чНомерДобавляемогоСегмента} Позиция=${оДобавляемыйСегмент.чTwitchПозиция} Длительность=${оДобавлено.чДлительность} Разрыв=${оДобавлено.лРазрыв}`);
	...
	function ОбновитьСписки(чЧерез) {
		...
		оОбещание.then(() => {
			...
			const чИнтервал = добавитьСегментыВОчередьИПолучитьПараметрыОбновленияСписков(РазобратьСписок(false, _моСписокВариантов[_чВыбранныйВариант].сАбсолютныйАдресСпискаСегментов, сРезультат));
	...
	function ПолучитьИнтервалОбновленияСпискаСегментов(кСегментовДобавлено) {
		if (_чСостояние === СОСТОЯНИЕ_ЗАВЕРШЕНИЕ_ТРАНСЛЯЦИИ || _чСостояние === СОСТОЯНИЕ_ОСТАНОВКА) {
			Проверить(_оПоследнийСписок === null && кСегментовДобавлено === 0);
			return 2e3;
		}
...
function Twitch(_сКодКанала) {
	...
	const АДРЕС_СЛЕЖЕНИЯ_ЗА_ПРОСМОТРОМ = 'https://spade.twitch.tv/track';
	...
	function ПолучитьИлиИзменитьПодпискуЗрителяНаКанал(чПодписка) {
		...
		м_Загрузчик.ЗагрузитьТекст(null, `https://api.twitch.tv/kraken/users/${_сИдЗрителя}/follows/channels/${_сИдКанала}`, ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ, 'проверка отслеживания канала', true, ЗАГОЛОВКИ_ЗАПРОСА).then(сРезультат => {
			let оРезультат;
			try {
			...
			м_События.ПослатьСобытие('twitch-полученыметаданныезрителя', {
				чПодписка: оРезультат.notifications ? ПОДПИСКА_УВЕДОМЛЯТЬ : ПОДПИСКА_НЕУВЕДОМЛЯТЬ
			});
		}).catch(ДобавитьОбработчикИсключений(пПричина => {
			if (пПричина === КОД_ОТВЕТА + 404) {
				...
				м_Журнал.Ой(`[Twitch] Не удалось проверить отслеживание канала. ${пПричина}`);
				м_События.ПослатьСобытие('twitch-полученыметаданныезрителя', {
					чПодписка: ПОДПИСКА_НЕДОСТУПНА
				});
			} else {
	...
	function неОтслеживатьКанал() {
		м_Загрузчик.Загрузить(null, 'POST', 'https://gql.twitch.tv/gql', ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ, {
			'Client-ID': ИДЕНТИФИКАТОР_КЛИЕНТА,
			'Content-Type': 'application/json',
			Authorization: `OAuth ${_сТокенЗрителя}`,
			'Accept-Language': 'en-US'
		}, создатьТелоЗапросаGql(`mutation($input: UnfollowUserInput!) {
		...
		}), 'не отслеживать канал', true, 'json').then(оРезультат => {
	...
	function отслеживатьКанал(чПодписка) {
		м_Загрузчик.Загрузить(null, 'POST', 'https://gql.twitch.tv/gql', ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ, {
			...
			Authorization: `OAuth ${_сТокенЗрителя}`,
			'Accept-Language': 'en-US'
		}, создатьТелоЗапросаGql(`mutation($input: FollowUserInput!) {
		...
		}), 'отслеживать канал', true, 'json').then(оРезультат => {
	...
	отправитьДанныеСлеженияЗаРекламой._лНужноОтправить = false;
	отправитьДанныеСлеженияЗаРекламой._сТипРекламы = '';
	отправитьДанныеСлеженияЗаРекламой._сТокенРекламы = '';
	function отправитьДанныеСлеженияЗаРекламой({сТипРекламы, сТокенРекламы}, лИдетРеклама) {
		if (отправитьДанныеСлеженияЗаРекламой._лНужноОтправить && (!лИдетРеклама || отправитьДанныеСлеженияЗаРекламой._сТипРекламы !== сТипРекламы || отправитьДанныеСлеженияЗаРекламой._сТокенРекламы !== сТокенРекламы)) {
			отправитьПросмотрРекламногоБлока(отправитьДанныеСлеженияЗаРекламой._сТипРекламы, отправитьДанныеСлеженияЗаРекламой._сТокенРекламы);
			отправитьДанныеСлеженияЗаРекламой._лНужноОтправить = false;
		}
		if (!отправитьДанныеСлеженияЗаРекламой._лНужноОтправить && лИдетРеклама && сТипРекламы && сТокенРекламы) {
			отправитьДанныеСлеженияЗаРекламой._лНужноОтправить = true;
			отправитьДанныеСлеженияЗаРекламой._сТипРекламы = сТипРекламы;
			отправитьДанныеСлеженияЗаРекламой._сТокенРекламы = сТокенРекламы;
		}
	}
	function отправитьПросмотрРекламногоБлока(сТипРекламы, сТокенРекламы) {
		const оЗаголовки = {
			'Content-Type': 'application/json',
			'Accept-Language': 'en-US',
			'Client-ID': ИДЕНТИФИКАТОР_КЛИЕНТА,
		...
		if (_сТокенЗрителя) {
			оЗаголовки.Authorization = `OAuth ${_сТокенЗрителя}`;
		}
		м_Загрузчик.Загрузить(null, 'POST', 'https://gql.twitch.tv/gql', 3e4, оЗаголовки, создатьТелоЗапросаGql(`mutation($input: RecordAdEventInput!) {
			...
			input: {
				...
				eventPayload: JSON.stringify({
					stitched: true,
					roll_type: сТипРекламы.toLowerCase(),
					player_mute: false,
					player_volume: 1,
					visible: true
				}),
				radToken: сТокенРекламы
			}
		}), `${сТипРекламы} ${сТокенРекламы.slice(-10)}`, false, 'json').then(оРезультат => {
	...
	function ПолучитьАбсолютныйАдресСпискаВариантов(ооОтменаОбещания, bHttp = false) {
		const ТОКЕН_ПРОТУХНЕТ_ЧЕРЕЗ = 18 * 60 * 1e3;
		...
		return прочестьПеченьки().then(() => {
			...
			const оЗаголовки = {
				...
				'Content-Type': 'application/json',
				'Accept-Language': 'en-US',
				'Device-ID': _сИдУстройства
			};
			if (_сТокенЗрителя) {
				оЗаголовки.Authorization = `OAuth ${_сТокенЗрителя}`;
			}
			return м_Загрузчик.Загрузить(ооОтменаОбещания, 'POST', 'https://gql.twitch.tv/gql', ЗАГРУЖАТЬ_СПИСОК_ВАРИАНТОВ_НЕ_ДОЛЬШЕ, оЗаголовки, создатьТелоЗапросаGql(`query(
						$login: String!
			...
			return ПолучитьАбсолютныйАдресСпискаВариантов._сАдрес = `${bHttp ? 'http' : 'https'}://usher.ttvnw.net/api/channel/hls/${encodeURIComponent(_сКодКанала)}.m3u8?allow_source=true&allow_audio_only=true&supported_codecs=avc1&token=${encodeURIComponent(сТокен)}&sig=${encodeURIComponent(сПодпись)}`;
		});
	}
	function получитьУникальныйИдентификаторУстройства() {
		const с = м_Настройки.Получить('чСлучайноеЧисло').toFixed(20).slice(2);
		return с + с.slice(0, 12);
	}
	...
	function ОбновитьМетаданныеТрансляции(чЧерез) {
		...
		Ждать(_ооОбновлениеМетаданных, чЧерез).then(() => {
			return м_Загрузчик.Загрузить(_ооОбновлениеМетаданных, 'POST', 'https://gql.twitch.tv/gql', ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ, {
				'Client-ID': ИДЕНТИФИКАТОР_КЛИЕНТА,
				'Content-Type': 'application/json',
				'Accept-Language': 'en-US'
			}, создатьТелоЗапросаGql(`query($id: ID!, $all: Boolean!) {
						user(id: $id) {
							broadcastSettings {
							...
							login
							stream {
								archiveVideo @include(if: $all) {id}
								createdAt
								id
								type
								viewersCount
				...
				id: _сИдКанала,
				all: _сИдТрансляции === ''
			}), 'метаданные трансляции', true, 'json');
		}).then(оРезультат => {
			...
			if (_сИдТрансляции === '' || _сИдТрансляции === сИдТрансляции) {
				...
				const сАдресИгры = цепочка(oUser, 'broadcastSettings', 'game', 'name');
				if (сАдресИгры) {
					оМетаданные.сАдресИгры = ПолучитьАдресКаталогаИгры(сАдресИгры);
				}
	...
	const отправитьДанныеСлеженияЗаПросмотром = ДобавитьОбработчикИсключений(() => {
		...
		м_Загрузчик.Загрузить(null, 'POST', АДРЕС_СЛЕЖЕНИЯ_ЗА_ПРОСМОТРОМ, ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ, null, оОтправить, 'слежение за просмотром', false, 'none').catch(пПричина => {
			if (typeof пПричина == 'string') {
	...
	function СоздатьКлип() {
		...
		const сОтправить = создатьТелоЗапросаGql(`mutation($input: CreateClipInput!) {
			...
			for (let оРасширение of моРасширения) {
				if (оРасширение.enabled) {
					switch (оРасширение.id) {
					  ...
					  case /*! Chrome */ 'aiimboljphncldaakcnapfolgnjonlea':
