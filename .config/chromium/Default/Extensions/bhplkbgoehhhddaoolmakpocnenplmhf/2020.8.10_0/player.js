...
const м_Чат = (() => {
 ...
 //! <iframe>
	...
	//! Remove HTTP response headers that block chat loading into <iframe>.
		...
		м_Журнал.Окак(`[Чат] Обрабатываю заголовки ответа ${response.url}`);
			...
			switch (responseHeaders[i].name.toLowerCase()) {
			  ...
			  case 'content-security-policy':
				if (/frame-ancestors/i.test(responseHeaders[i].value)) {
					м_Журнал.Окак(`[Чат] Удаляю заголовок Content-Security-Policy: ${responseHeaders[i].value}`);
					responseHeaders.splice(i, 1);
					modified = true;
				}
			}
	...
	function ВставитьПанель() {
		...
		chrome.webRequest.onHeadersReceived.addListener(onHeadersReceived, {
			urls: [ 'https://www.twitch.tv/popout/*/chat', 'https://www.twitch.tv/popout/*/chat?*', 'https://www.twitch.tv/embed/*/chat', 'https://www.twitch.tv/embed/*/chat?*', 'https://www.twitch.tv/*/chat?*' ],
			types: [ 'sub_frame' ],
			tabId: г_чИдВкладки
			...
			for (const оМедиаустройство of моМедиаустройства) {
				...
				if (оМедиаустройство.kind === 'audiooutput') {
					...
					if (оМедиаустройство.deviceId && оМедиаустройство.label) {
						кНастоящихУстройств += Number(оМедиаустройство.deviceId !== УСТРОЙСТВО_ПО_УМОЛЧАНИЮ && оМедиаустройство.deviceId !== УСТРОЙСТВО_ДЛЯ_ОБЩЕНИЯ);
...
function Twitch(_сКодКанала) {
	...
	function ПолучитьАбсолютныйАдресСпискаВариантов(ооОтменаОбещания, bHttp = false) {
		...
		return ПрочестьПеченькуАвторизацииЗрителя().then(() => {
			...
			if (ПолучитьАбсолютныйАдресСпискаВариантов.сПроигрыватель === '') {
				ПолучитьАбсолютныйАдресСпискаВариантов.сПроигрыватель = Math.random() < .5 ? `&platform=web&player_backend=flash&player_type=${Math.random() < .5 ? 'site' : 'twitch_everywhere'}` : `&platform=mobile_web&player_backend=mediaplayer&player_type=${Math.random() < .5 ? 'popout' : 'twitch_everywhere'}`;
			}
	...
	const отправитьДанныеСлеженияЗаПросмотром = ДобавитьОбработчикИсключений(() => {
		...
		м_Загрузчик.Загрузить(null, 'POST', 'https://video-edge-c5c834.pdx01.abs.hls.ttvnw.net/v1/segment/CsSiNbC-7inQN1966EWsT9wJ5RkaSQfokp3DHReBuJ1klk2SnWPbV4kWl2OqtrBvBJsnZV005u3nnNIYazkIaWRNTEbmw0A1TkwllBv2OCfZj8mHdasOwkv6Ue_2nSBUT-UH-i9BlNOup3J235Pq68v2mcNUMMZORXY9TSU-WBujk5ogtVfpa6Zpjb6m1B1IjcFK5POM63arhoU01E6qhEuFcgwu6hfmcxmwQsB-2uoxg3l4wKYPQkwbOvNn3HCNUMWGoGMA5ES257H4Xy9QvVxaIQgzoG6qqBcLiKDs_4HvyeIjDhAV5ybeoJ92CqWp7LA4olpuSJVgjWhaFZug43XcttuumyqCcwsFBa3uF90pMpuFboRwlAwNHmqRXy12YF2AABO4Z9uMdj3dVNb0drjeXqkS-DdI4LeEwXraYK4XWYFbSB6P20EEuqFiVT3DNnXtc0XDmn1e9Q1I8ZAxWHFqEY65XFYAhsUwAWWyjRGmxp5E7fJI6eWXgAUM-m3Fjqn6nouos2PaDKRQm3Gh3Uo431IPCrbnSj64OZVdotEIpg2c7SoqKkqxMXB42o4ZVvVfzr-MHYogiDP8YjmOuSwQ_471orNMNCmeNNsFxkrLIaL948PpJq6pe1j8Tq67Y3bP11AomZ_ypT5qNb4rShFHoV6HZoOo1Pf3sZvGs3rTCTUd0IIJWrARHUgqSULGbv1kQhQ533M0Bo-NwxPZX7nzknvD1UTlCw6AhV4m0SCpFEZ-Z1uEf43OCL1KeNScAEWrZ5JZKEOcIJ7BBwwpbzJaJ3Fw.ts', ЗАГРУЖАТЬ_МЕТАДАННЫЕ_НЕ_ДОЛЬШЕ, null, оОтправить, 'слежение за просмотром', false, 'none').catch(пПричина => {
			if (typeof пПричина == 'string') {
